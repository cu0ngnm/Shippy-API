...
let Order = {
  ...
  GetSellerHistory:function(sellerId, statusId, callback){
    if(statusId == 1 || statusId == 3){
      return connection.query('select `order`.order_code, `order`.seller_phone, `order`.buyer_phone, seller.seller_name ,`order`.order_price, `order`.order_fee, `order`.order_description, `order`.category_id, ' +
